...
p_webdriver.prototype.run = function(desired) {
    ...
    console.log("---------------"+this.desired.length);
    console.log(this.desired);
    
    for (var i=0; i < this.desired.length; i++){
	//Create browser
	_this.browsers[i] = wd.remote(this.remoteWdConfig);
	
	//Set up callbacks
	for (var k in this.desired[i].callbacks){
	    _this.browsers[i].on(k, this.desired[i].callbacks[k]);
	}

	//Run tests for current browser
	Wd( function() {_this.test(_this.browsers[i], _this.desired[i]) });
    }
}
